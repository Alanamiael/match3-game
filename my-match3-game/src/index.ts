...
class Game {
  ...
  private initBoard(): void {
    this.board = Array.from({ length: CONFIG.gridSize }, (_, row) =>
      Array.from({ length: CONFIG.gridSize }, (_, _col) => ({
  ...
  private animateSmoothFall(done: () => void): void {
    const animate = () => {
      ...
      for (let row = 0; row < CONFIG.gridSize; row++) {
        for (let col = 0; col < CONFIG.gridSize; col++) {
          ...
          if (cell.isAnimating) {
            ...
            const distance = cell.targetRow - cell.sourceRow;
            const totalTime = Math.abs(distance) * CONFIG.fallSpeed;
            const progress = Math.min(elapsed / totalTime, 1);

            const currentY = cell.sourceRow + distance * progress;

            this.ctx.drawImage(
              this.images[cell.type],
              col * CONFIG.cellSize,
              currentY * CONFIG.cellSize,
              CONFIG.cellSize,
              CONFIG.cellSize
            );
  ...
  private animateInitialFall(done: () => void): void {
    ...
    const animate = (time: number) => {
      ...
      for (let row = 0; row < CONFIG.gridSize; row++) {
        for (let col = 0; col < CONFIG.gridSize; col++) {
          const type = this.board[row][col].type;
          if (type !== -1) {
            const y = -CONFIG.cellSize + CONFIG.cellSize * row * progress;
            this.ctx.drawImage(
              this.images[type],
              col * CONFIG.cellSize,
              y,
  ...
  private drawBoard(): void {
    ...
    for (let row = 0; row < CONFIG.gridSize; row++) {
      for (let col = 0; col < CONFIG.gridSize; col++) {
        const cell = this.board[row][col];
        if (cell.type !== -1 && !cell.isAnimating) {
          this.ctx.drawImage(
            this.images[cell.type],
            col * CONFIG.cellSize,
            row * CONFIG.cellSize,
